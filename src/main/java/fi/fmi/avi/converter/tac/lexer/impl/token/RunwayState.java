...
public class RunwayState extends RegexMatchingLexemeVisitor {
    ...
    static {
        {
            final HashMap<RunwayStateDeposit, RunwayDeposit> tmp = new HashMap<>();
            final HashMap<RunwayDeposit, RunwayStateDeposit> tmp2 = new HashMap<>();
            ...
            tmp.forEach((lexemeDeposit, apiDeposit) -> tmp2.put(apiDeposit, lexemeDeposit));
            apiToRunwayStateDeposit = Collections.unmodifiableMap(tmp2);
        }

        {
            final HashMap<RunwayStateContamination, RunwayContamination> tmp = new HashMap<>();
            final HashMap<RunwayContamination, RunwayStateContamination> tmp2 = new HashMap<>();
            ...
            tmp.forEach((lexemeContamination, apiContamination) -> tmp2.put(apiContamination, lexemeContamination));
            apiToRunwayStateContamination = Collections.unmodifiableMap(tmp2);
        }

        {
            final HashMap<BreakingAction, AviationCodeListUser.BrakingAction> tmp = new HashMap<>();
            final HashMap<AviationCodeListUser.BrakingAction, BreakingAction> tmp2 = new HashMap<>();
            ...
            tmp.forEach((lexemeBreakingAction, apiBreakingAction) -> tmp2.put(apiBreakingAction, lexemeBreakingAction));
    ...
    public static RunwayStateDeposit convertAPIToRunwayStateDeposit(final RunwayDeposit deposit) {
        return apiToRunwayStateDeposit.get(deposit);
    }

    public static RunwayContamination convertRunwayStateContaminationToAPI(final RunwayStateContamination contamination) {
        return runwayStateContaminationToAPI.get(contamination);
    }

    public static RunwayStateContamination convertAPIToRunwayStateContamination(final RunwayContamination contamination) {
        return apiToRunwayStateContamination.get(contamination);
    }

    public static AviationCodeListUser.BrakingAction convertBreakingActionToAPI(final BreakingAction breakingAction) {
        return breakingActionToAPI.get(breakingAction);
    }

    public static BreakingAction convertAPIToBreakingAction(final AviationCodeListUser.BrakingAction brakingAction) {
        return apiTobreakingAction.get(brakingAction);
    }

    private static void appendFrictionCoeffOrBreakingAction(final String coded, final HashMap<RunwayStateReportType, Object> values)
        ...
        if ("//".equals(coded)) {
            ...
            final int fcbaValue = Integer.parseInt(coded);
            if (fcbaValue == 99) {
                ...
                final BreakingAction ba = BreakingAction.forCode(fcbaValue);
                if (ba != null) {
    ...
    protected String getRunwayDesignationMatch(final Matcher match) {
    ...
    public void visitIfMatched(final Lexeme token, final Matcher match, final ConversionHints hints) {
        ...
        final HashMap<RunwayStateReportType, Object> values = new HashMap<>();
        Lexeme.Status status = Lexeme.Status.OK;
        String msg = null;

        final Object runwayDesignation = getRunwayDesignation(getRunwayDesignationMatch(match));
        ...
        if (match.group(MATCH_DEPOSIT_CODE) != null && match.group(MATCH_CONTAMINATION_CODE) != null && match.group(MATCH_DEPTH_CODE) != null) {
            ...
            final String depthCode = match.group(MATCH_DEPTH_CODE);
            if ("00".equals(depthCode)) {
                values.put(RunwayStateReportType.DEPTH_OF_DEPOSIT, 1);
                ...
                final int depthCodeNumber = Integer.parseInt(depthCode);
                if (depthCodeNumber <= 90) {
                    ...
                    switch (depthCodeNumber) {
                        case 92:
                            values.put(RunwayStateReportType.DEPTH_OF_DEPOSIT, 10);
                            break;
                        case 93:
                            values.put(RunwayStateReportType.DEPTH_OF_DEPOSIT, 15);
                            break;
                        case 94:
                            values.put(RunwayStateReportType.DEPTH_OF_DEPOSIT, 20);
                            break;
                        case 95:
                            values.put(RunwayStateReportType.DEPTH_OF_DEPOSIT, 25);
                            break;
                        case 96:
                            values.put(RunwayStateReportType.DEPTH_OF_DEPOSIT, 30);
                            break;
                        case 97:
                            values.put(RunwayStateReportType.DEPTH_OF_DEPOSIT, 35);
                            break;
                        case 98:
                            values.put(RunwayStateReportType.DEPTH_OF_DEPOSIT, 40);
    ...
    public enum RunwayStateDeposit {
        ...
        public static RunwayStateDeposit forCode(final char code) {
            for (final RunwayStateDeposit w : values()) {
                if (w.code == code) {
                    return w;
                }
            }
    ...
    public enum RunwayStateContamination {
        ...
        public static RunwayStateContamination forCode(final char code) {
            for (final RunwayStateContamination w : values()) {
                if (w.code == code) {
                    return w;
                }
            }
    ...
    public enum RunwayStateReportType {
        DEPOSITS, CONTAMINATION, DEPTH_OF_DEPOSIT, UNIT_OF_DEPOSIT, FRICTION_COEFFICIENT, BREAKING_ACTION, REPETITION, ALL_RUNWAYS, CLEARED, DEPTH_MODIFIER
    }
    ...
    public enum BreakingAction {
        ...
        public static BreakingAction forCode(final int code) {
            for (final BreakingAction w : values()) {
                if (w.code == code) {
                    return w;
                }
            }
    ...
    public static class Reconstructor extends FactoryBasedReconstructor {
        ...
        public <T extends AviationWeatherMessageOrCollection> Optional<Lexeme> getAsLexeme(final T msg, final Class<T> clz, final ReconstructorContext<T> ctx)
            ...
            final Optional<fi.fmi.avi.model.metar.RunwayState> state = ctx.getParameter("state", fi.fmi.avi.model.metar.RunwayState.class);
            if (state.isPresent()) {
                final boolean annex3_16th = ctx.getHints().containsValue(ConversionHints.VALUE_SERIALIZATION_POLICY_ANNEX3_16TH);
                final String str = buildRunwayStateToken(state.get(), annex3_16th);
                return Optional.of(this.createLexeme(str, RUNWAY_STATE));
            }
            return Optional.empty();
        }

        private String buildRunwayStateToken(final fi.fmi.avi.model.metar.RunwayState state, final boolean annex3_16th) throws SerializingException {
            ...
            if (state.isCleared()) {
                ...
                if (state.getDeposit().isPresent()) {
                    final RunwayStateDeposit deposit = convertAPIToRunwayStateDeposit(state.getDeposit().get());
                    if (deposit == null) {
                ...
                if (state.getContamination().isPresent()) {
                    final RunwayStateContamination contamination = convertAPIToRunwayStateContamination(state.getContamination().get());
        ...
        private String getFrictionCoefficient(final fi.fmi.avi.model.metar.RunwayState state) throws SerializingException {
            final Optional<AviationCodeListUser.BrakingAction> action = state.getBrakingAction();
            final Optional<Double> friction = state.getEstimatedSurfaceFriction();
            ...
            if (action.isPresent()) {
                final BreakingAction tmp = convertAPIToBreakingAction(action.get());
                if (tmp == null) {
            ...
            if (friction.isPresent()) {
                final double value = friction.get();
        ...
        private String getDepthOfDeposit(final fi.fmi.avi.model.metar.RunwayState state) throws SerializingException {
            final Optional<NumericMeasure> measure = state.getDepthOfDeposit();
            final Optional<fi.fmi.avi.model.AviationCodeListUser.RelationalOperator> operator = state.getDepthOperator();
            ...
            final int value = measure.get().getValue().intValue();
        ...
        private String getRunwayDesignator(final fi.fmi.avi.model.metar.RunwayState state, final boolean annex3_16th) throws SerializingException {
            ...
            if (state.isRepetition()) {
                ...
                final Optional<RunwayDirection> rwd = state.getRunwayDirection();
                if (!rwd.isPresent()) {
                ...
                if (annex3_16th) {
                    ...
                    final boolean rightSide = designator.endsWith("R");
