...
public class CloudLayer extends RegexMatchingLexemeVisitor {
        ...
        if ("///".equals(match.group("amount")) && "///".equals(match.group("height"))) {
            ...
            if (match.group("amount") != null) {
                if ("///".equals(match.group("amount"))) {
                    ...
                    final CloudCover cloudCover = CloudCover.forCode(match.group("amount"));
                    if (cloudCover != null) {
                        token.identify(LexemeIdentity.CLOUD);
                ...
                final CloudCover cloudCover = CloudCover.forCode(match.group("nocloud"));
                token.identify(LexemeIdentity.CLOUD);
                token.setParsedValue(COVER, cloudCover);
            }
            ...
            if (match.group("type") != null) {
                if ("///".equals(match.group("type"))) {
                    ...
                    final CloudType type = CloudType.forCode(match.group("type"));
    ...
    public static class Reconstructor extends FactoryBasedReconstructor {
        ...
        public <T extends AviationWeatherMessageOrCollection> Optional<Lexeme> getAsLexeme(final T msg, final Class<T> clz, final ReconstructorContext<T> ctx)
            ...
            final Optional<fi.fmi.avi.model.CloudLayer> layer = ctx.getParameter("layer", fi.fmi.avi.model.CloudLayer.class);
            ...
            if (TAF.class.isAssignableFrom(clz)) {
                ...
                final Optional<TAFBaseForecast> baseFct = ctx.getParameter("forecast", TAFBaseForecast.class);
                if (baseFct.isPresent()) {
                    cFct = baseFct.get().getCloud();
                } else {
                    final Optional<TAFChangeForecast> changeFct = ctx.getParameter("forecast", TAFChangeForecast.class);
                    if (changeFct.isPresent()) {
                        cFct = changeFct.get().getCloud();
                    }
                }
                if (cFct.isPresent()) {
                    final Optional<Boolean> verticalVisibility = ctx.getParameter("verticalVisibility", Boolean.class);
                ...
                if (trend.isPresent()) {
                    if (trend.get().getCloud().isPresent()) {
                        final fi.fmi.avi.model.CloudForecast cloud = trend.get().getCloud().get();
                        if (verticalVisibility.isPresent() && verticalVisibility.get()) {
                    ...
                    final Optional<ObservedClouds> obsClouds = metar.getClouds();
        ...
        private String getCloudLayerToken(final fi.fmi.avi.model.CloudLayer layer) throws SerializingException {
            ...
            final Optional<NumericMeasure> base = layer.getBase();
            final Optional<fi.fmi.avi.model.AviationCodeListUser.CloudType> type = layer.getCloudType();
            final Optional<CloudAmount> amount = layer.getAmount();
